...
lex = function(expression) {
  ...
  for (index = 0; index < expression.length;) {
    try {
      token_start = index;
      state = lex_machine.get_next(expression.slice(index, index + 1));
      do {
        index++;
...
mark = function(exp) {
  var tokens = typeof exp == 'string' ? lex(exp) : exp,
    token_index,
    tk,
    marked = "";
...
value_clean_disambiguation = function(tokens) {
  var index,
    previous,
    token,
    value;
...
pull_tree = function(tokens, start, level) {
  start = start? start : 0,
  level = level? level : 0;
  var index = start,
    end = tokens.length,
      ...
      if (op[0].test(token.name) && token.type === op[1]) {
        if (token.type === 'bracket') {
          if (/^o_/.test(token.name)) {  // open
            pull_tree(tokens, index + 1, level + 1);
...
var compile = function(expression) {
  var dirty_tokens = lex(expression);
  var clean_tokens = value_clean_disambiguation(dirty_tokens);
  var compiled = pull_tree(clean_tokens);
  return compiled[0].fn;
};


module.exports = {
  lex_machine: lex_machine,
  lex: lex,
  mark: mark,
  value_clean_disambiguation: value_clean_disambiguation,
  compile: compile
}
